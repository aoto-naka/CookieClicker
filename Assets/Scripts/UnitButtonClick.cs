 ...
 */
public class UnitButtonClick : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    void Update(){
        if(cookie.GetComponent<CookieQuantityManager>().ReturnCookiesQuantity() < (ulong)price){
            // 所持クッキー数が価格よりすくなかったらボタンを灰色に
            this.GetComponent<Image>().color = new Color(120f/255f, 120f/255f, 120f/255f);
        }
        else{
    ...
    {
        if(cookie.GetComponent<CookieQuantityManager>().ReturnStage()+1 >= unitNumber){
            //Debug.Log(name);
    ...
    public void OnClick()
    {
        ...
        if(cookie.GetComponent<CookieQuantityManager>().ReturnStage()+1 >= unitNumber){
            ...
            audioSource.PlayOneShot(clickSound);
                ...
                if(cookie.GetComponent<CookieQuantityManager>().ReturnStage() < unitNumber){
                    cookie.GetComponent<CookieQuantityManager>().SetStage(unitNumber);
                }
            }
